...
# NEW: For nozzles/instruments attached to large vessels
VESSEL_EXPANSION_PX = 20 
...
def generate_full_cypher_script(yolo_input_path, lines_csv_path, cypher_output_path):
    ...
    for i in range(len(symbol_nodes)):
        ...
        if node1['label'] == 'Rectangle_small':
            expansion = PROXIMITY_EXPANSION_PX
        elif node1['label'] == 'Rectangle_large':
            expansion = VESSEL_EXPANSION_PX
        else:
            continue # This node does not initiate proximity checks

        n1_x_min = node1['x_center'] - node1['width'] / 2 - expansion
        n1_y_min = node1['y_center'] - node1['height'] / 2 - expansion
        n1_x_max = node1['x_center'] + node1['width'] / 2 + expansion
        n1_y_max = node1['y_center'] + node1['height'] / 2 + expansion
        expanded_box1 = (n1_x_min, n1_y_min, n1_x_max, n1_y_max)

        for j in range(len(symbol_nodes)):
            if i == j: continue # Don't compare a node to itself
...
if __name__ == "__main__":
    ...
    cypher_file = "pid_graph_complete2.cypher"
